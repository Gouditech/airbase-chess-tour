...
async function getAccessToken() {
  ...
  return { messaging: tokens.access_token, jwt: jwtClient };
}

async function getAdminPin(jwtClient) {
  // Read adminPin directly from Firebase REST API
  const tokenRes = await jwtClient.getAccessToken();
  const token = tokenRes.token;
  const url = `${DB_URL}/adminPin.json?access_token=${token}`;
...
exports.handler = async function(event) {
  ...
  try {
    const { messaging: accessToken, jwt: jwtClient } = await getAccessToken();
